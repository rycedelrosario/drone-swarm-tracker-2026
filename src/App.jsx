...
export default function App(){
 ...
 const trackBufferRef = useRef([])

 // WebSocket Connection Effect + Buffer Maintenance
 useEffect(() => {
   ...
   ws.onmessage = (event) => {
     try {
       ...
       if (data.type === 'tracks_snapshot') {
         ...
         const currentMediaTime = vid ? vid.currentTime : (Date.now() / 1000);
        
         trackBufferRef.current.push({
           time: currentMediaTime,
           tracks: data.tracks
         });
         if (trackBufferRef.current.length > 900) {
           trackBufferRef.current.shift();
         }
       }
     } catch (err) {
 ...
 // Sync Loop (Radar <-> Video Playback Time)
 useEffect(() => {
   let animFrameId;
   const syncLoop = () => {
     const vid = videoRef.current;
     if (vid && trackBufferRef.current.length > 0) {
       const curTime = vid.currentTime;
       let closest = trackBufferRef.current[0];
       let minDiff = Math.abs(curTime - closest.time);
       for (let i = 1; i < trackBufferRef.current.length; i++) {
         const diff = Math.abs(curTime - trackBufferRef.current[i].time);
         if (diff < minDiff) {
           minDiff = diff;
           closest = trackBufferRef.current[i];
         }
       }
       if (closest && closest.tracks) {
         setSyncedTracks(closest.tracks);
       }
     }
 ...
 const tracks = useMemo(() => {
   if (syncedTracks && syncedTracks.length > 0) return syncedTracks;
   if (wsTracks && wsTracks.length > 0) return wsTracks;
   return simulatedTracks;
 }, [syncedTracks, wsTracks, simulatedTracks]);

 // Video Overlay Canvas Draw Loop with Strict Letterbox Alignment
 // Video Overlay Canvas Draw Loop
 useEffect(() => {
  ...
  const videoH = video.videoHeight || 576;
  ...
  tracks.slice(0, 2).forEach(tr => {
    const isSelected = tr.id === selectedId;

    if (tr.bbox) {
      let bx = offsetX + tr.bbox[0] * renderW;
      let by = offsetY + tr.bbox[1] * renderH;
      let bw = (tr.bbox[2] - tr.bbox[0]) * renderW;
      let bh = (tr.bbox[3] - tr.bbox[1]) * renderH;

      // Enforce minimum target indicator dimensions when zoomed out
      const MIN_SIZE = 16;
      if (bw < MIN_SIZE) {
        bx -= (MIN_SIZE - bw) / 2;
        bw = MIN_SIZE;
      ...
      ctx.strokeStyle = isSelected ? '#38bdf8' : tr.flags.includes('LOST') ? '#ef4444' : '#22c55e';
      ctx.lineWidth = isSelected ? 2 : 1.5;
      ctx.strokeRect(bx, by, bw, bh);

      ctx.fillStyle = isSelected ? '#38bdf8' : '#eaf2ff';
      ctx.font = '10px monospace';
      ctx.fillText(tr.callsign, bx, by - 4);
    }
  });
}, [tracks, selectedId]);
   ...
   <div className="shell">
     <div className="topbar">
       ...
       <div className="actions">
         ...
         <button className="btn primary" onClick={handlePlayPause}>{playing ? 'Pause' : 'Play'}</button>
       ...
       <div className="panel">
         <div className="panelHeader">
           <div className="panelTitle">
             ...
             <div className="d">Live target detection video stream</div>
           </div>
           <div className="kpi"><span>DVIDS • Perdix demo</span></div>
         </div>
         <div className="panelBody">
           <div className="videoBox" style={{ position: 'relative', overflow: 'hidden' }}>
             <video
               ref={videoRef}
             ...
             <div className="hud" style={{ pointerEvents: 'none' }}>
               ...
               <div className="tag tagTR"><b>INTEGRITY</b> • live tracking</div>
               <div className="tag tagBL"><b>NOTE</b> • relative bearing & range active</div>
             </div>
           </div>
           <div className="controlsRow">
             <button className="btn primary" onClick={handlePlayPause}>{playing ? 'Pause' : 'Play'}</button>
             <button className="btn" onClick={rewind}>-5s</button>
             <button className="btn" onClick={fastFwd}>+5s</button>
             <select className="select" value={speed} onChange={(e) => {
             ...
             <div className="small">Playback • Synced Radar</div>
           </div>
         </div>
       </div>
             ...
             {selected ? (
               <>
                 ...
                 <div className="inspectorGrid" style={{ marginTop: 10 }}>
                   ...
                   <div className="cardMini"><div className="k">Altitude</div><div className="v">{selected.altitude_m ? `${fmt(selected.altitude_m,0)} m` : selected.alt_band}</div></div>
                 ...
                 {filtered.map(tr => {
                   ...
                   return (
                     <tr key={tr.id} onClick={() => setSelectedId(tr.id)} style={{ background: isSel ? 'rgba(56,189,248,.08)' : undefined }}>
                       ...
                       <td><span className="badge">{tr.altitude_m ? `${fmt(tr.altitude_m,0)}m` : tr.alt_band}</span></td>
                     </tr>
                   );
                 })}
               </tbody>

  ...
  useEffect(() => {
    ...
    if (tracks && tracks.length > 0) {
      tracks.forEach(tr => {
        ...
        const rad = ((tr.bearing - 90) * Math.PI) / 180;
        const cx = (canvas.width / 2) + (canvas.width / 2) * tr.range_u * Math.cos(rad);
        const cy = (canvas.height / 2) + (canvas.height / 2) * tr.range_u * Math.sin(rad);

        const boxSize = isSelected ? 28 : 20;
        ...
        ctx.fillText(tr.callsign, cx - boxSize / 2, cy - boxSize / 2 - 4);
      });
    }
  }, [tracks, selectedId]);
